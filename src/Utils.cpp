...
std::vector<Arc> Utils::generateArcs(const std::vector<Node>& nodes,
    ...
    std::cout<< wireless_arcs_filename << std::endl;
    ...
    while (std::getline(file, line)) {
        if (line.empty() || line.find_first_not_of(" \t\r\n") == std::string::npos) {
            break; // Thoát vòng lặp khi gặp dòng trống
        }
        std::cout << count++;
        std::stringstream ss(line);
...
Parameters Utils::readParams(const std::string& filename) {
    ...
    Parameters params(battery_capacity , 0.2 * battery_capacity, energy_consumption,
                     vehicle_speed, wireless_cost, time_cost,
                     battery_capacity, 1e6);
    // Parameters params(battery_capacity * 1000, 0.2 * battery_capacity, energy_consumption,
    //              vehicle_speed, wireless_cost, time_cost,
    //              battery_capacity * 1000, 1e6);
...
std::vector<int> Utils::generateInitialRoute(const std::vector<Node>& nodes,
    ...
    // Separate customers and stations
    for (const auto& node : nodes) {
        if (node.getType() == NodeType::CUSTOMER) {
    ...
    // Build route with charging stations
    std::vector<int> route = {start_depot_id};

    // Insert charging station after every 2-3 customers
    // int count = 0;
    for (int customer : customers) {
        route.push_back(customer);
        // count++;
        // if (count % 2 == 0 && !stations.empty()) {
        //     route.push_back(stations[count % stations.size()]);
        // }
    }

...
VNS::VNS(const std::vector<int>& initial_route,
    ...
    : env(), init_route(initial_route), current_route(initial_route), graph(g), charge_options(c_options),
      params(p), max_iterations(max_iter), rng(r), operator_weights_{0.4, 0.3, 0.2, 0.1, 0.1},
...
Route VNS::run() {
    Route S0 = init_route;
    if (!isValidRoute(S0.getNodeIds(), graph, init_route)) {
        S0 = localSearch(S0);
    }

//    ProblemInfos initial_result;
    Route S = S0;
    Route Sbest = S0;
    ...
    for (int iter = 0; iter < max_iterations && noImprovement < max_no_improvement; ++iter) {
        ...
        Route S_prime = shake(S, op, graph, params);

        if (quickFeasibilityCheck(S_prime, graph, params)) {
            S_prime = localSearch(S_prime);
            double C_S_prime = S_prime.getTotalCost();

            if (C_S_prime < C_Sbest) {
                Sbest = S_prime;
                S = S_prime;
                C_Sbest = C_S_prime;
...
//                updateRoute(S, initial_result);
                ...
                if ((C_Sbest - C_S_prime) / C_Sbest >= 0.0001) {
                    noImprovement = 0;
                } else {
                    noImprovement++;
                }
            } else if (C_S_prime < S.getTotalCost()) {
                S = S_prime;
                noImprovement++;
            } else {
                noImprovement++;
        ...
        if ((iter + 1) % 10 == 0) {
            ...
            int i = 0;
            for (auto const & decision : Sbest.getWirelessDecisions()) {
                if (decision) {
                    std::cout << "Wireless charging used between nodes "
                              << Sbest.getNodeIds()[i] << " and "
                              << Sbest.getNodeIds()[i + 1] << "\n";
                }
                i++;
            }
            std::cout << "------------------------\n";
...
//    return current_soc >= p.getMinSoc();
//}

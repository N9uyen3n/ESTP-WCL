...
Route MILP::optimize(const std::vector<int>& initial_nodes,
    ...
    try {
        ...
        for (size_t i = 0; i < node_ids.size(); ++i) {
            for (size_t j = 0; j < node_ids.size(); ++j) {
                if (i != j && graph.findArc(node_ids[i], node_ids[j])) {
                    ...
                    // THÊM RÀNG BUỘC THEO TÀI LIỆU (Dạng >=)
                    if (arc->getIsWireless()) {
                        model.add(y_a[j] >= y_d[i] - params.getEnergyConsumption() * dist +
                                          arc->getWirelessChargeRate() * s_z[i][j] -
                                          params.getBatteryCapacity() * (1 - x[i][j])); // Ràng buộc (16)
                    } else {
                        model.add(y_a[j] >= y_d[i] - params.getEnergyConsumption() * dist -
                                          params.getBatteryCapacity() * (1 - x[i][j])); // Ràng buộc (15)
                    }

                    model.add(y_a[j] >= params.getMinSoc()); // Giữ nguyên ràng buộc SOC tối thiểu
                }
            }
        }
        ...
        cplex.exportModel("model.lp"); // Xuất mô hình để kiểm trc
...
Route MILP::optimizeNoWCL(const std::vector<int>& initial_nodes,
    ...
    try {
        ...
        // Auxiliary Variables for Linearization (only for phi_w)
        IloArray<IloNumVarArray> phi_w(env, node_ids.size());
        for (size_t i = 0; i < node_ids.size(); ++i) {
            int node_i = node_ids[i];
            phi_w[i] = IloNumVarArray(env, charge_options[node_i].size(), 0, IloInfinity, ILOFLOAT);
        ...
        auto end_time = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time).count();
        std::cout << "MILP::optimizeNoWCL: Success, cost = " << result.cost << ", Time = " << duration << " s\n";
        return result_route;
    } catch (IloException& e) {

...
VNS2::VNS2(const std::vector<int>& initial_route,
           const Graph& g,
           ...
           std::mt19937& r)
    : env(),
      init_route(initial_route),
      current_route(initial_route),
      graph(g),
      charge_options(c_options),
      params(p),
      max_iterations(max_iter),
      rng(r),
      operator_weights_{0.4, 0.3, 0.2, 0.05, 0.05}, // 5 toán tử
...
Route VNS2::run() {
    Route S0 = init_route;
    if (!quickFeasibilityCheck(S0, graph, params)) {
        S0 = localSearch(S0);
    }
    ...
    int noImprovement = 0;
    const int max_no_improvement = 140;
    ...
    for (int iter = 0; iter < max_iterations && noImprovement < max_no_improvement; ++iter) {
        ...
        Route S_prime = solveSubproblem(S_.getNodeIds(), charge_options, params);
        double C_S_;
        bool is_feasible = (S_prime.getNodeIds() != S_.getNodeIds() ||
                           S_prime.getTotalCost() != std::numeric_limits<double>::infinity());

        if (is_feasible && quickFeasibilityCheck(S_prime, graph, params)) {
            S_prime = localSearch(S_prime);
            C_S_ = S_prime.getTotalCost();
        } else {
            // Nếu không khả thi, thử thêm trạm sạc
            bool inserted = insertStation();
            if (inserted) {
                S_prime = solveSubproblem(S_.getNodeIds(), charge_options, params);
                is_feasible = (S_prime.getNodeIds() != S_.getNodeIds() ||
                               S_prime.getTotalCost() != std::numeric_limits<double>::infinity());
                if (is_feasible && quickFeasibilityCheck(S_prime, graph, params)) {
                    S_prime = localSearch(S_prime);
                    C_S_ = S_prime.getTotalCost();
...
Route VNS2::solveSubproblem(const std::vector<int>& initial_nodes,
...
bool VNS2::isValidRoute(const std::vector<int>& route, const Graph& g, const std::vector<int>& initial_nodes) {
...
bool VNS2::quickFeasibilityCheck(const Route& route, const Graph& g, const Parameters& p) {
    ...
    for (size_t i = 0; i < nodes.size() - 1; ++i) {
        ...
        if (std::find(station_ids.begin(), station_ids.end(), nodes[i + 1]) != station_ids.end()) {
...
int VNS2::selectOperatorWeighted() {
    std::discrete_distribution<> dist(operator_weights_.begin(), operator_weights_.end());
    return dist(rng);
}

void VNS2::updateRoute(Route& route, const ProblemInfos& result) {
...
Route VNS2::shake(const Route& route, int neighborhood, const Graph& g, const Parameters& p) {
    switch (neighborhood) {
...
Route VNS2::twoOpt(const Route& route, const Graph& g, const Parameters& p) {
...
Route VNS2::swapNodes(const Route& route, const Graph& g, const Parameters& p) {
    Route new_route = route;
    auto nodes = new_route.getNodeIds();
    if (nodes.size() <= 3) return new_route;
    std::uniform_int_distribution<> dist(1, nodes.size() - 2);
...
Route VNS2::relocate(const Route& route, const Graph& g, const Parameters& p) {
    Route new_route = route;
    auto nodes = new_route.getNodeIds();
    if (nodes.size() <= 3) return new_route;
    std::uniform_int_distribution<> dist(1, nodes.size() - 2);
...
bool VNS2::insertStation() {
    // Lấy danh sách trạm sạc từ đồ thị
    const auto& stations = graph.getStations();

    // Lọc các trạm chưa có trong station_ids
    std::vector<int> available_stations;
    for (int station : stations) {
        if (std::find(station_ids.begin(), station_ids.end(), station) == station_ids.end()) {
            available_stations.push_back(station);
        }
    }

    // Nếu không còn trạm khả dụng, trả về false
    if (available_stations.empty()) {
        return false;
    }

    // Chọn ngẫu nhiên một trạm
    std::uniform_int_distribution<> dist(0, available_stations.size() - 1);
    int station_id = available_stations[dist(rng)];

    // Thêm trạm vào station_ids
    station_ids.push_back(station_id);
    return true;
}

bool VNS2::removeStation() {
    // Nếu station_ids rỗng, trả về false
    if (station_ids.empty()) {
        return false;
    }

    // Chọn ngẫu nhiên một trạm để xóa
    std::uniform_int_distribution<> dist(0, station_ids.size() - 1);
    int idx = dist(rng);

    // Xóa trạm
    station_ids.erase(station_ids.begin() + idx);
    return true;
}

Route VNS2::localSearch(const Route& current_route) {
    std::vector<int> initial_nodes = current_route.getNodeIds();
    Route optimized_route = solveSubproblem(initial_nodes, charge_options, params);

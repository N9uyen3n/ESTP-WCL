...
std::vector<int> generateInitialRoute(const std::vector<Node>& nodes, const std::vector<Arc>& arcs, const Params& params) {
    int n = nodes.size();
    int starting_depot = 0; // Starting depot is node 0 (D0)
    int ending_depot = 16;  // Ending depot is node 16 (D1)
    ...
    // Identify customers and charging stations
    std::vector<int> customers;
    std::vector<int> charging_stations;
    for (const auto& node : nodes) {
        if (node.type == "c") customers.push_back(node.id); // Customer nodes
        else if (node.type == "f") charging_stations.push_back(node.id); // Charging station nodes
    }
    ...
    while (!unvisited_customers.empty()) {
        ...
        if (!found) {
            ...
            if (!feasible_stations.empty()) {
                ...
                route.push_back(next);
                current_SOC = params.Q; // Charge to full capacity
                current = next;
            } else {
                std::cout << "Cannot reach any charging station from " << current << " with SOC " << current_SOC << "\n";
                break; // Route becomes infeasible
            }
        }
    }

    // After visiting all customers, go to ending depot (node 16)
    while (true) {
        if (can_reach(current, ending_depot, current_SOC, arc_matrix, params)) {
            route.push_back(ending_depot);
            break; // Successfully reached ending depot
        } else {
            // Find nearest feasible charging station
            std::vector<std::pair<double, int>> feasible_stations;
            for (int k : charging_stations) {
                if (can_reach(current, k, current_SOC, arc_matrix, params)) {
                    double dist = arc_matrix[current][k].dij;
            ...
            if (!feasible_stations.empty()) {
                ...
                std::cout << "Cannot reach ending depot from " << current << " with SOC " << current_SOC << "\n";
                break; // Route becomes infeasible
            }
        }
    }
...
int main() {
    ...
    try {
        ...
        Params params = CSVReader::readParams("D:\\Work\\NEULab\\ESTP-WCL-Test3\\data\\Input\\params.csv");
        printParamsInfo(params);

        // Read and print node information
        std::vector<Node> nodes = CSVReader::readNodes("D:\\Work\\NEULab\\ESTP-WCL-Test3\\data\\Input\\nodes.csv");
        printNodesInfo(nodes);

        // Read and print arc information
        std::vector<Arc> arcs = CSVReader::generateArcs(nodes,
            "D:\\Work\\NEULab\\ESTP-WCL-Test3\\data\\Input\\wireless_arcs.csv", params);
        printArcsInfo(arcs);
        ...
        std::vector<std::vector<ChargingOption>> charge_options = CSVReader::readChargingOptions(nodes,
            "D:\\Work\\NEULab\\ESTP-WCL-Test3\\data\\Input\\charging_options.csv");

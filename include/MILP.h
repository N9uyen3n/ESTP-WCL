...
struct SubproblemResult {
    ...
    std::vector<std::pair<int, int>> arc_wireless_decisions; // Wireless charging decisions for each arc wireless>
...
public:
    ...
    Route optimizeNoWCL(const std::vector<int>& initial_nodes,
                          const Graph& graph,
                          const std::vector<std::vector<ChargingOption>>& charge_options,
                          const Parameters& params);
